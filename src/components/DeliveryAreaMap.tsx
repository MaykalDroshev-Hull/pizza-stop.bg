...
export default function DeliveryAreaMap({ apiKey }: DeliveryAreaMapProps) {
  ...
  const checkAddressCoverage = async (address: string) => {
    ...
    if (isInLovechCity || hasLovechPostalCode) {
      isInCoverage = true
      deliveryZone = `Ловеч град (2 €. / ${formatBGNPrice(convertToBGN(2))})`
    } else if (isInVillage) {
      isInCoverage = true
      deliveryZone = `Разширена зона (5 €. / ${formatBGNPrice(convertToBGN(5))})`
    }
  ...
  const initMap = () => {
    ...
    // Define Lovech city area (2 EUR delivery) - Golden color
    const lovechArea = [
    ...
    // Define extended area (5 EUR delivery) - Blue color
    ...
    // Add extended area polygon (5 EUR delivery) - Blue color
    ...
    // Add Lovech city area polygon (2 EUR delivery) - Golden color
    new window.google.maps.Polygon({
      paths: lovechArea,
      ...
      <div className="container">
        <h2 className={styles.sectionTitle}>Райони за доставка</h2>
           ...
           <div className={styles.infoCard}>
             ...
             <h3>Ловеч - 2 €. <span className="text-muted text-sm font-normal">({formatBGNPrice(convertToBGN(2))})</span></h3>
             <p>Доставка в град Ловеч и близките райони - Гозница, Продимчец, Вилна зона - Бабаковец, Синантепе</p>
           </div>
           
                       <div className={styles.infoCard}>
              <MapPin className={styles.infoIcon} />
              <h3>Разширена зона - 5 €. <span className="text-muted text-sm font-normal">({formatBGNPrice(convertToBGN(5))})</span></h3>
           ...
           <div className={styles.mapLegend}>
             <div className={styles.legendItem}>
               ...
               <span>Ловеч - 2 €. <span className="text-muted text-xs">({formatBGNPrice(convertToBGN(2))})</span> доставка</span>
             </div>
             <div className={styles.legendItem}>
               <div className={styles.legendColor} style={{ backgroundColor: '#3b82f6' }}></div>
               <span>Разширена зона - 5 €. <span className="text-muted text-xs">({formatBGNPrice(convertToBGN(5))})</span> доставка</span>
             </div>
           </div>
         </div>
         ...
         <div className={styles.deliveryDetails}>
           ...
           <ul>
            <li><strong>Работно време:</strong><br/> Понеделник – Събота: 11:00 – 21:00<br/>Неделя: почивен ден</li>
            <li><strong>Онлайн поръчки:</strong><br/> Понеделник – Събота: 11:30 - 20:30<br/>Неделя: почивен ден</li>
             ...
             <li><strong>Доставка извън града: 5 €. <span className="text-muted text-sm font-normal">({formatBGNPrice(convertToBGN(5))})</span></strong></li>
